...
void solve(long long N, std::vector<long long> L, std::vector<long long> R){
    ...
    vector<pair<int,int>> vpans;
    rep(i,N){
        vp[i] = make_pair(L[i], R[i]);
    }
    sort(all(vp));

    vpans.push_back(vp[0]);
    rep(i,N-1){
        if(vp[i].second>=vp[i+1].first)
            vpans[vpans.size()-1].second = max(vpans[vpans.size()-1].second,vp[i+1].second) ;
        else{
            vpans.push_back(vp[i+1]);
        }
    }
    for(auto a: vpans){
